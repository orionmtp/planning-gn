...
if (mysqli_num_rows($result) ==1) {
	...
	if($gn==0) {
		...
		if ($row['running']==0) {
			...
			$situation=date_format($now,"Y-m-d H:i");
			if (isset($_POST['changerevent'])){
				...
				$id=$_POST['obj'];
				if ($avance==1){
					$debut1=date_format(date_sub($debut,$differ),"Y-m-d H:i");
					$prepa1=date_format(date_sub(date_sub($prepa,$differ),date_sub($debut,$differ)),"Y-m-d H:i");
					$fin1=date_format(date_sub(date_sub($fin,$differ),date_sub($debut,$differ)),"Y-m-d H:i");
				}
				else {
					$debut1=date_format(date_add($debut,$differ),"Y-m-d H:i");
					$prepa1=date_format(date_sub(date_add($prepa,$differ),date_add($debut,$differ)),"Y-m-d H:i");
					$fin1=date_format(date_sub(date_add($fin,$differ),date_add($debut,$differ)),"Y-m-d H:i");
				}
			...
			if (mysqli_num_rows($result) > 0) {
				...
				while($row = mysqli_fetch_assoc($result)) {
					...
					if ($activ==1) {
						...
						$fin_event=date_create($row['fin']);
						echo '<tr><td><a  target="_blank" href="event.php?event='. $row["id"]  .'">'. $row["nom"] .'</a></td><td>'. $row["priorite"] .'</td><form method="POST" action="running.php?serial='.$serial.'"><td><input type="datetime" value="'.date_format($deb_prepa,"Y-m-d H:i").'" name="prepa"></td><td><input type="datetime" name="debut" value="'.date_format($deb_event,"Y-m-d H:i").'"></td></form></tr>';
					}
				}
			}
			echo "</table><br>\n";
			...
			echo "a venir<br>\n";
			if ($avance==1) $sql="select event.id,subtime(addtime(gn.debut,event.debut),delta) as debut1,subtime(subtime(addtime(gn.debut,event.debut),event.prepa),delta) as prepa1, subtime(addtime(addtime(gn.debut,event.debut),event.duree),delta) as fin,event.nom,priorite  from event inner join gn on gn.id=event.gn where  gn='$gn' and subtime(subtime(addtime(gn.debut,event.debut),event.prepa),delta)>'$situation' order by prepa1";
			else $sql="select event.id,addtime(addtime(gn.debut,event.debut),delta) as debut1,addtime(subtime(addtime(gn.debut,event.debut),event.prepa),delta) as prepa1, addtime(addtime(addtime(gn.debut,event.debut),event.duree),delta) as fin,event.nom,priorite  from event inner join gn on gn.id=event.gn where  gn='$gn' and addtime(subtime(addtime(gn.debut,event.debut),event.prepa),delta)>'$situation' order by prepa1";
			$result=mysqli_query($db,$sql)  or die(mysqli_error($db));
			if (mysqli_num_rows($result) > 0) {
				...
				while($row = mysqli_fetch_assoc($result)) {
					...
					if ($activ==1) {
						...
						$deb_prepa=date_create($row['prepa1']);
						$fin_event=date_create($row['fin']);
						echo '<tr><td><a  target="_blank" href="event.php?event='. $row["id"]  .'">'. $row["nom"] .'</a></td><td>'. $row["priorite"] .'</td><form method="POST" action="running.php?serial='.$serial.'"><td><input type="datetime" value="'.date_format($deb_prepa,"Y-m-d H:i").'" name="prepa"></td><td><input type="datetime" name="fin" value="'.date_format($fin_event,"Y-m-d H:i").'"></td></form></tr>';
					}
				}
			}
			echo "</table><br>\n";
			...
			echo "termines<br>\n";
			if ($avance==1) $sql="select event.id,subtime(addtime(gn.debut,event.debut),delta) as debut1,subtime(subtime(addtime(gn.debut,event.debut),event.prepa),delta) as prepa1, subtime(addtime(addtime(gn.debut,event.debut),event.duree),delta) as fin,event.nom,priorite  from event inner join gn on gn.id=event.gn where gn='$gn' and subtime(addtime(addtime(gn.debut,event.debut),event.duree),delta)<'$situation' order by fin";
			else $sql="select event.id,addtime(addtime(gn.debut,event.debut),delta) as debut1,addtime(subtime(addtime(gn.debut,event.debut),event.prepa),delta) as prepa1, addtime(addtime(addtime(gn.debut,event.debut),event.duree),delta) as fin,event.nom,priorite  from event inner join gn on gn.id=event.gn where gn='$gn' and addtime(addtime(addtime(gn.debut,event.debut),event.duree),delta)<'$situation' order by fin";
			$result=mysqli_query($db,$sql)  or die(mysqli_error($db));
			if (mysqli_num_rows($result) > 0) {
			...
			$result=mysqli_query($db,$sql)  or die(mysqli_error($db));
			echo '<table><tr><td>titre</td><td>personnage</td><td>reussite</td></tr>';
			if (mysqli_num_rows($result) > 0) {
				while($row = mysqli_fetch_assoc($result)) {
					if ($row['role']!=0) {
						$role_temp=$row['role'];
